import * as React from "react";
import { Path, Svg } from "react-native-svg";
import type { IconProps } from "../types.js";

export const SiTextualHex = "FFFFFF";
export const SiTextualTitle = "Textual";
export const SiTextualSlug = "textual";

export function SiTextual({
  size = 24,
  color = "black",
  title,
  ...props
}: IconProps): React.ReactElement {
  const resolvedColor = color === "default" ? `#${SiTextualHex}` : color;

  return (
    <Svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      accessibilityLabel={title ?? SiTextualTitle}
      {...props}
    >
      <Path fill={resolvedColor} d="M13.746 2.731H24l-1.722 3.873-3.143 1.768H17l-5.182 10.552-3.128 2.345H5.283l.747-11.216H1.67L0 6.296l2.511-1.884h8.246zM2.709 5.006l-1.45 1.088h8.952l.249-1.088zM.825 6.69l1.23 2.77h4.611l-.747 11.215h.941L10.074 6.69zm7.567 13.985 5.232-12.897h5.24l1.23-2.77H11.07L7.469 20.675zm14.02-17.35h-8.508l-1.935 1.087h8.505z" />
    </Svg>
  );
}
